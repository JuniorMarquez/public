...
/* Controllers */
  ...
  app.controller('CarouselDemoCtrl', ['$scope', '$http',function($scope,$http) {
    ...
    $scope.myInterval = 5000;
     $http.get('http://localhost:1346/slide').then(function (resp) {
...
inscripcion.status='pendiente';
    ...
    inscripcion.identificacion=item.item.identificacion;
    ...
    $http.post('http://localhost:1346/inscripcion/', inscripcion)
    $modalInstance.close();
$scope.popInscripcion();
  ...
  $http.get('http://localhost:1345/electoral').then(function (resp) {
    $scope.listadoElectoral = resp.data.results;
  });
    $http.get('http://localhost:1345/deporte').then(function (resp) {
    $scope.listadoDeporte = resp.data.results;
  });

     $http.get('http://localhost:1345/municipio').then(function (resp) {
    $scope.municipios = resp.data.results;
  });
      $http.get('http://localhost:1345/directiva').then(function (resp) {
    $scope.listadoDirectiva = resp.data.results;
  });
    $http.get('http://localhost:1345/comite').then(function (resp) {
    $scope.listadoComite = resp.data.results;
  });
  $http.get('http://localhost:1345/universidad').then(function (resp) {
...
$scope.consultaEvento=function(item){
  ...
  $scope.datosMiembro={};
  $http.get('http://localhost:1345/evento/'+identificador).success(function(respuesta){        
    item=respuesta;
...
$scope.consultaMiembro=function(item){
  ...
  $scope.datosMiembro={};
  $http.get('http://localhost:1345/miembro/'+identificador).success(function(respuesta){        
    item=respuesta;
    $scope.item=item;
  });
  item=$scope.item;
...
$scope.okMision= function (item) {
  var configAct = {};
      ...
      $http.put('http://localhost:1345/configuracion/'+MyService.data.idConfig , configAct);       
      $modalInstance.close();
    };
$scope.entrar2=function(item){
  var iden = MyService.data.idConfig;
  var datosCuenta=[];
  $http.get('http://localhost:1345/configuracion').success(function(respuesta){
...
$scope.borrarDeporte=function(item){
   var idDeporte=item.id;
      $http.delete('http://localhost:1345/deporte/'+idDeporte , item)
      $modalInstance.dismiss('cancel');
};
$scope.borrarElectoral=function(item){
   var idElectoral=item.id;
      $http.delete('http://localhost:1345/electoral/'+idElectoral , item)
      $modalInstance.dismiss('cancel');
};

$scope.borrarDirectiva=function(item){
   var idDirectiva=item.id;
      $http.delete('http://localhost:1345/directiva/'+idDirectiva , item)
      $modalInstance.dismiss('cancel');
};
$scope.borrarComite=function(item){
   var idComite=item.id;
      $http.delete('http://localhost:1345/comite/'+idComite , item)
      $modalInstance.dismiss('cancel');
};

$scope.borrarUniversidad=function(item){
   var idUniversidad=item.id;
      $http.delete('http://localhost:1345/universidad/'+idUniversidad , item)
      $modalInstance.dismiss('cancel');
};
$scope.borrarMunicipio=function(item){
   var idMunicipio=item.id;
      $http.delete('http://localhost:1345/municipio/'+idMunicipio , item)
      $modalInstance.dismiss('cancel');
};
...
$scope.okEspecialidad = function (item) {
    ...
    $scope.okDirectiva = function (item) {
      ...
      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1345/directiva/' ,item);       
      $modalInstance.close();
    };
    $scope.okComite = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1345/comite/' ,item);       
      $modalInstance.close();
    };
    $scope.okDeporte = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1345/deporte/' ,item);       
      $modalInstance.close();
    };
    $scope.okElectoral = function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1345/electoral/' ,item);       
      $modalInstance.close();
    };
...
$scope.okAprobacion = function (item) {
    ...
    $http.put('http://localhost:1346/suscripcion/'+identificador, suscripcionAct);
    $modalInstance.close();
    setTimeout(function() { 
      $http.get('http://localhost:1346/suscripcion/' ).success(function(respuesta){
 ...
 $scope.okEdicion = function (item) {
    ...
    $http.put('http://localhost:1345/miembro/'+identificador, miembroAct);
    $modalInstance.close();
    setTimeout(function() { 
...
$scope.okUniversidad= function (item) {
      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1345/universidad/' ,item);       
      $modalInstance.close();
    };
    $scope.okMunicipio= function (item) {

      item.idUsuario=MyService.data.idUsuario;
      $http.post('http://localhost:1345/municipio/' ,item);       
      $modalInstance.close();
    };
...
$scope.okActividad = function (item) {
      ...
      $http.post('http://localhost:1345/evento/' ,item);
    ...
    $scope.okEditarActividad = function (item) {
      ...
      item.idUsuario=MyService.data.idUsuario;
      item.tipo=MyService.data.tipoActividad;
      $http.put('http://localhost:1345/evento/'+item.id ,item);       
      $modalInstance.close();
   $scope.popEditEvento();
     setTimeout(function() {
        $state.go('app.dashboard-v1');
 ...
 $scope.popInscripcion = function(){
  ...
  $scope.popBorrado = function(){
    ...
    $scope.okBorrarMiembro = function (item,timeout) { 
      ...
      $http.delete('http://localhost:1345/miembro/'+dato); 
      $scope.popBorrado();
      $modalInstance.close();
      setTimeout(function() {
        $state.go('app.dashboard-v1');
    ...
    $scope.okConfirmProfesional = function (item) { 
      ...
      $http.delete('http://localhost:1345/miembro/'+idProfesional , item)
      $scope.items = null;
      $scope.item = null; 
      $modalInstance.close();
    };
